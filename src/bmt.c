...
#define BMT_CHECK_INPUT(_name)\
	...
	{\
		...
		{\
			errno = BMT_STATUS_BAD_NAME;\
			return;\
		}\
	} while(0)

#define BMT_FOR_EACH(node, cursor)\
...
static BMT_node _bmt_alloc_node(BMT_tag tag, const SPchar* name, SPsize length, const void* value, SPbool copyValue)
{
	...
	switch(tag)
	{
		case BMT_TAG_NULL: break;
		case BMT_TAG_BYTE: node->payload.tag_byte = *(const SPbyte*) value; break;
		case BMT_TAG_SHORT: node->payload.tag_short = *(const SPshort*) value; break;
		case BMT_TAG_INT: node->payload.tag_int = *(const SPint*) value; break;
		case BMT_TAG_LONG: node->payload.tag_long = *(const SPlong*) value; break;
		case BMT_TAG_FLOAT: node->payload.tag_float = *(const SPfloat*) value; break;
		case BMT_TAG_DOUBLE: node->payload.tag_double = *(const SPdouble*) value; break;
		case BMT_TAG_STRING:
		{
...
static SPsize _bmt_length_of(BMT_tag tag)
{
	switch(tag)
	{
		case BMT_TAG_BYTE: return sizeof(SPbyte);
		case BMT_TAG_SHORT: return sizeof(SPshort);
		case BMT_TAG_INT: return sizeof(SPint);
		case BMT_TAG_LONG: return sizeof(SPlong);
		case BMT_TAG_FLOAT: return sizeof(SPfloat);
		case BMT_TAG_DOUBLE: return sizeof(SPdouble);
		case BMT_TAG_NULL: break;
	}
	return 0;
}
...
static SPbool _bmt_copy_payload_from_node(const BMT_node src, BMT_node dst)
{
	if(src && dst)
	{
		...
		{
			case BMT_TAG_NULL: break;
			case BMT_TAG_BYTE: dst->payload.tag_byte = src->payload.tag_byte; break;
			case BMT_TAG_SHORT: dst->payload.tag_short = src->payload.tag_short; break;
			case BMT_TAG_INT: dst->payload.tag_int = src->payload.tag_int; break;
			case BMT_TAG_LONG: dst->payload.tag_long = src->payload.tag_long; break;
			case BMT_TAG_FLOAT: dst->payload.tag_float = src->payload.tag_float; break;
			case BMT_TAG_DOUBLE: dst->payload.tag_double = src->payload.tag_double; break;
			case BMT_TAG_STRING:
			{
			...
			default:
			{
				SPsize length = _bmt_length_of_list(src->payload.tag_object);
...
static SPbool _bmt_copy_payload_from_memory(BMT_node node, BMT_tag tag, SPsize length, const void* value, SPbool copyValue)
{
	if(node)
	{
		switch(tag)
		{
			case BMT_TAG_NULL: break;
			case BMT_TAG_BYTE: node->payload.tag_byte = *(const SPbyte*) value; node->length = sizeof(SPbyte); break;
			case BMT_TAG_SHORT: node->payload.tag_short = *(const SPshort*) value; node->length = sizeof(SPshort); break;
			case BMT_TAG_INT: node->payload.tag_int = *(const SPint*) value; node->length = sizeof(SPint); break;
			case BMT_TAG_LONG: node->payload.tag_long = *(const SPlong*) value; node->length = sizeof(SPlong); break;
			case BMT_TAG_FLOAT: node->payload.tag_float = *(const SPfloat*) value; node->length = sizeof(SPfloat); break;
			case BMT_TAG_DOUBLE: node->payload.tag_double = *(const SPdouble*) value; node->length = sizeof(SPdouble); break;
			case BMT_TAG_STRING:
			{
			    node->length = length;
...
static BMT_node _bmt_copy_tree(const BMT_node n)
{
    ...
    if(n)
    {
        ...
        tree->major = _bmt_copy_tree(n->major);
        tree->minor = _bmt_copy_tree(n->minor);
...
static BMT_node _bmt_copy_list(const BMT_node n)
{
	...
	if(n)
	{
		SP_ASSERT(bmt_IsList(n), "Expected list node");
		if(n->red && !n->parent)
		{
			list = bmt_AllocList();
...
SPbool _bmt_is_equal(const BMT_node a, const BMT_node b)
{
    ...
    {
        case BMT_TAG_NULL: break;
        case BMT_TAG_BYTE: BMT_COMPARE_GENERIC(tag_byte); break;
        case BMT_TAG_SHORT: BMT_COMPARE_GENERIC(tag_short); break;
        case BMT_TAG_INT: BMT_COMPARE_GENERIC(tag_int); break;
        case BMT_TAG_LONG: BMT_COMPARE_GENERIC(tag_long); break;
        case BMT_TAG_FLOAT:
        {
...
{      
    ...
    return (a && b) ? (_bmt_is_tree_equal(a->major, b->major) && _bmt_is_tree_equal(a->minor, b->minor)) : SP_TRUE;
}
...
void _bmt_delete_tree_impl(BMT_node tree)
{
	...
	{	
		...
		_bmt_delete_tree_impl(major);
		_bmt_delete_tree_impl(minor);
	}
}
...
static BMT_node _bmt_search_impl(const BMT_node tree, const SPchar* name)
{
	if(tree)
	{
		...
		if(cmp > 0)
			return _bmt_search_impl(tree->major, name);
		else
			return _bmt_search_impl(tree->minor, name);
	}
	return NULL;
}
...
BMT_node _bmt_search(const BMT_node tree, const SPchar* name)
{
	...
	if(bmt_IsTree(tree))
	{
		ret = _bmt_search_impl(tree, name);
	}
	return ret;
}

static BMT_node _bmt_insert_data(BMT_node* head, BMT_node node, const SPchar* name, BMT_tag tag, SPsize length, const void* value, SPbool copyValue)
{
	if(!node)
	{
	...
	if((node->nameLength == 0 || node->name == NULL) || node->tag == BMT_TAG_NULL)
	{
		...
		_bmt_copy_payload_from_memory(node, tag, length, value, copyValue);
		node->tag = tag;
...
const SPchar* _bmt_tag_to_str(BMT_tag tag)
{
	switch(tag)
	{
		case BMT_TAG_ROOT: return "tree";
		case BMT_TAG_BYTE: return "byte";
		case BMT_TAG_SHORT: return "short";
		case BMT_TAG_INT: return "int";
		case BMT_TAG_LONG: return "long";
		case BMT_TAG_FLOAT: return "float";
		case BMT_TAG_DOUBLE: return "double";
		case BMT_TAG_STRING: return "string";
		case BMT_TAG_LIST: return "multi-list";
		case BMT_TAG_ROOT_LIST: return "tree-list";
		case BMT_TAG_BYTE_LIST: return "byte-list";
		case BMT_TAG_SHORT_LIST: return "short-list";
		case BMT_TAG_INT_LIST: return "int-list";
		case BMT_TAG_LONG_LIST: return "long-list";
		case BMT_TAG_FLOAT_LIST: return "float-list";
		case BMT_TAG_DOUBLE_LIST: return "double-list";
		case BMT_TAG_STRING_LIST: return "string-list";
	}
	return "NULL";
}
...
static void _bmt_print(const BMT_node tree, SPbuffer* buffer, int level, SPbool printTreeData)
{
	if(tree)
	{
		...
		switch(tree->tag)
		{
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): %hhd\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_byte);
				else
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): %hd\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_short);
				else
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): %d\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_int);
				else
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): %lld\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_long);
				else
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): %f\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_float);
				else
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): %f\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_double);
				else
			...
			{
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (\"%s\"): \"%s\"\n", color, rank, _bmt_tag_to_str(tree->tag), tree->name, tree->payload.tag_string);
				else
			...
			default:
			{
				SP_ASSERT(tree->length == _bmt_length_of_list(tree->payload.tag_object), "Expected length of %lld for list but got length of %lld", _bmt_length_of_list(tree->payload.tag_object), tree->length);
				if(printTreeData)
					_bmt_bprintf(buffer, "(%c%c) %s (%lld elements) (\"%s\"):\n", color, rank, _bmt_tag_to_str(tree->tag), tree->length, tree->name);
				else
		...
		BMT_FOR_EACH(list, cursor)
		{
			switch(cursor->tag)
			{
				...
				case BMT_TAG_STRING_LIST:
				{
					...
					_bmt_bprintf(buffer, "%s (%lld elements):\n", _bmt_tag_to_str(cursor->tag), _bmt_length_of_list(cursor->payload.tag_object));
				...
				default:
				{
					_bmt_print(cursor, buffer, level, SP_FALSE);
					break;
				}
			}
		}
...
void _bmt_print_tree(const BMT_node tree, SPbuffer* buffer, int level)
{
	if(tree)
	{
		_bmt_print(tree, buffer, level, SP_TRUE);
		_bmt_print_tree(tree->major, buffer, level + 1);
		_bmt_print_tree(tree->minor, buffer, level + 1);
	}
}

const SPchar* bmt_ToString(const BMT_node node)
{
...
void bmt_Print(const BMT_node node)
{
	if(node)
	{
		const SPchar* str = bmt_ToString(node);
		printf("%s", str);
		free((void*)str);
	}
}
...
void bmt_InsertByte(BMT_node* tree, const SPchar* name, SPbyte value)
{
	...
	_bmt_insert_data(tree, *tree, name, BMT_TAG_BYTE, sizeof(SPbyte), &value, SP_TRUE);
}

void bmt_InsertShort(BMT_node* tree, const SPchar* name, SPshort value)
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_SHORT, sizeof(SPshort), &value, SP_TRUE);
}

void bmt_InsertInt(BMT_node* tree, const SPchar* name, SPint value)
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_INT, sizeof(SPint), &value, SP_TRUE);
}

void bmt_InsertLong(BMT_node* tree, const SPchar* name, SPlong value)
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_LONG, sizeof(SPlong), &value, SP_TRUE);
}

void bmt_InsertFloat(BMT_node* tree, const SPchar* name, SPfloat value)
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_FLOAT, sizeof(SPfloat), &value, SP_TRUE);
}

void bmt_InsertDouble(BMT_node* tree, const SPchar* name, SPdouble value)
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_DOUBLE, sizeof(SPdouble), &value, SP_TRUE);
}
...
void bmt_InsertString(BMT_node* tree, const SPchar* name, const SPchar* value)
{
	...
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_STRING, strlen(value), value, SP_TRUE);
}
...
static void _bmt_insert_list(BMT_node* tree, const SPchar* name, BMT_tag tag, SPsize length, const void* values)
{
	...
	_bmt_insert_data(tree, *tree, name, tag, _bmt_length_of_list(list), list, SP_FALSE);
}
...
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_ROOT, 0, NULL, SP_FALSE);
}

void bmt_CreateList(BMT_node* tree, const SPchar* name)
{
	BMT_CHECK_INPUT(name);
	_bmt_insert_data(tree, *tree, name, BMT_TAG_LIST, 0, NULL, SP_FALSE);
}
...
void bmt_Insert(BMT_node* tree, const SPchar* name, const BMT_node value)
{
    ...
    if(tag == BMT_TAG_NULL)
    {
    	errno = BMT_STATUS_BAD_TAG;
    	return;
    }
    
	_bmt_insert_data(tree, *tree, name, tag, bmt_IsList(value) ? _bmt_length_of_list(value) : 0, value, SP_TRUE);
}
...
SPlong bmt_GetNumber(const BMT_node tree, const SPchar* name)
{
	...
	if(n)
	{
		switch(n->tag)
		{
			case BMT_TAG_BYTE: return n->payload.tag_byte;
			case BMT_TAG_SHORT: return n->payload.tag_short;
			case BMT_TAG_INT: return n->payload.tag_int;
			case BMT_TAG_LONG: return n->payload.tag_long;
		}
	}
	return 0LL;
}
...
SPdouble bmt_GetDecimal(const BMT_node tree, const SPchar* name)
{
	...
	if(n)
	{
		switch(n->tag)
		{
			case BMT_TAG_FLOAT: return n->payload.tag_float;
			case BMT_TAG_DOUBLE: return n->payload.tag_double;
		}
	}
	return 0.0;
}
...
static void _bmt_set_value(BMT_node tree, BMT_tag tag, const SPchar* name, const void* value)
{
	...
	if(n && n->tag == tag)
	{
		switch(tag)
		{
			case BMT_TAG_BYTE: n->payload.tag_byte = *((const SPbyte*) value); break;
			case BMT_TAG_SHORT: n->payload.tag_short = *((const SPshort*) value); break;
			case BMT_TAG_INT: n->payload.tag_int = *((const SPint*) value); break;
			case BMT_TAG_LONG: n->payload.tag_long = *((const SPlong*) value); break;
			case BMT_TAG_FLOAT: n->payload.tag_float = *((const SPfloat*) value); break;
			case BMT_TAG_DOUBLE: n->payload.tag_double = *((const SPdouble*) value); break;
			case BMT_TAG_STRING:
			{
...
BMT_node bmt_ToList(BMT_tag tag, SPsize length, const void* data)
{
	...
	for(SPsize i = 0; i < length; i++)
	{
		...
		{
			case BMT_TAG_BYTE: bmt_AppendByte(&list, *(((const SPbyte*) data) + i)); break;
			case BMT_TAG_SHORT: bmt_AppendShort(&list, *(((const SPshort*) data) + i)); break;
			case BMT_TAG_INT: bmt_AppendInt(&list, *(((const SPint*) data) + i)); break;
			case BMT_TAG_LONG: bmt_AppendLong(&list, *(((const SPlong*) data) + i)); break;
			case BMT_TAG_FLOAT: bmt_AppendFloat(&list, *(((const SPfloat*) data) + i)); break;
			case BMT_TAG_DOUBLE: bmt_AppendDouble(&list, *(((const SPdouble*) data) + i)); break;
			case BMT_TAG_STRING:
			{
...
void _bmt_insert_multi_list(BMT_node* _list, BMT_tag tag, SPsize length, const void* values, SPbool copyValue)
{
	...
	if(!list)
	{
		*_list = bmt_AllocList();
		(*_list)->tag = tag;
		_bmt_copy_payload_from_memory(*_list, tag, length, values, copyValue);
	}
	else
	{
		if(!bmt_IsList(list))
		...
		else
		{
			if(list->tag == tag)
			{
				...
				_bmt_copy_payload_from_memory(cursor->major, tag, length, values, copyValue);
			}
		}
	}
}
...
{
	_bmt_insert_multi_list(list, BMT_TAG_BYTE, sizeof(SPbyte), &value, SP_TRUE);
}

void bmt_AppendShort(BMT_node* list, SPshort value)
{
	_bmt_insert_multi_list(list, BMT_TAG_SHORT, sizeof(SPshort), &value, SP_TRUE);
}

void bmt_AppendInt(BMT_node* list, SPint value)
{
	_bmt_insert_multi_list(list, BMT_TAG_INT, sizeof(SPint), &value, SP_TRUE);
}

void bmt_AppendLong(BMT_node* list, SPlong value)
{
	_bmt_insert_multi_list(list, BMT_TAG_LONG, sizeof(SPlong), &value, SP_TRUE);
}

void bmt_AppendFloat(BMT_node* list, SPfloat value)
{
	_bmt_insert_multi_list(list, BMT_TAG_FLOAT, sizeof(SPfloat), &value, SP_TRUE);
}

void bmt_AppendDouble(BMT_node* list, SPdouble value)
{
	_bmt_insert_multi_list(list, BMT_TAG_DOUBLE, sizeof(SPdouble), &value, SP_TRUE);
}
...
void bmt_AppendString(BMT_node* list, const SPchar* value)
{
	...
	_bmt_insert_multi_list(list, BMT_TAG_STRING, strlen(value) + 1, value, SP_TRUE);
}
...
void bmt_AppendByteList(BMT_node* list, SPsize length, const SPbyte* values)
{
	...
	_bmt_insert_multi_list(list, BMT_TAG_BYTE_LIST, length, v, SP_FALSE);
}
...
void bmt_AppendShortList(BMT_node* list, SPsize length, const SPshort* values)
{
	...
	SP_ASSERT(v, "Expected list non-equal NULL");
	_bmt_insert_multi_list(list, BMT_TAG_SHORT_LIST, length, v, SP_FALSE);
}
...
void bmt_AppendIntList(BMT_node* list, SPsize length, const SPint* values)
{
	...
	SP_ASSERT(v, "Expected list non-equal NULL");
	_bmt_insert_multi_list(list, BMT_TAG_INT_LIST, length, v, SP_FALSE);
}
...
void bmt_AppendLongList(BMT_node* list, SPsize length, const SPlong* values)
{
	...
	SP_ASSERT(v, "Expected list non-equal NULL");
	_bmt_insert_multi_list(list, BMT_TAG_LONG_LIST, length, v, SP_FALSE);
}
...
void bmt_AppendFloatList(BMT_node* list, SPsize length, const SPfloat* values)
{
	...
	SP_ASSERT(v, "Expected list non-equal NULL");
	_bmt_insert_multi_list(list, BMT_TAG_FLOAT_LIST, length, v, SP_FALSE);
}
...
void bmt_AppendDoubleList(BMT_node* list, SPsize length, const SPdouble* values)
{
	...
	SP_ASSERT(v, "Expected list non-equal NULL");
	_bmt_insert_multi_list(list, BMT_TAG_DOUBLE_LIST, length, v, SP_FALSE);
}
...
void bmt_AppendStringList(BMT_node* list, SPsize length, const SPchar** values)
{
	...
	SP_ASSERT(v, "Expected list non-equal NULL");
	_bmt_insert_multi_list(list, BMT_TAG_STRING_LIST, _bmt_length_of_list(v), v, SP_FALSE);
}
...
void bmt_Append(BMT_node* list, const BMT_node value)
{
    ...
    _bmt_insert_multi_list(list, tag, bmt_IsTree(value) ? 0 : _bmt_length_of_list(value), value, SP_TRUE);
}
...
const SPchar* bmt_GetErrorInfo(BMT_status status)
{
    switch(status)
    {
        case BMT_STATUS_NO_MEMORY: return "Failed to allocate memory";
        case BMT_STATUS_READ_ERROR: return "Error while reading object";
        case BMT_STATUS_WRITE_ERROR: return "Error while writing object";
        case BMT_STATUS_BAD_NAME: return "Name was empty or already taken";
        case BMT_STATUS_BAD_VALUE: return "Given value was invalid";
        case BMT_STATUS_BAD_TAG: return "Given node had non-matching tag";
        case BMT_STATUS_NOT_A_TREE: return "Given node was not a tree-node";
        case BMT_STATUS_NOT_A_LIST: return "Given node was not a list-node";
    }
    return "Ok";
}
#if defined(SP_COMPILER_CLANG) || defined(SP_COMPILER_GNUC)
